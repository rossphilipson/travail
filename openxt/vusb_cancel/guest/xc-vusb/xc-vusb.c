...
static void
vusb_flush_ring(struct vusb_device *vdev, bool force_notify)
{
	int notify;

	RING_PUSH_REQUESTS_AND_CHECK_NOTIFY(&vdev->ring, notify);

	if (notify || force_notify)
		...
		bool force_notify)
{
	...
	vusb_flush_ring(vdev, force_notify);
}
		...
		enum vusb_internal_cmd cmd, u64 cancel_id)
{
	...
	vusb_put_ring(vdev, shadow, force_notify);
...
vusb_put_urb(struct vusb_device *vdev, struct vusb_urbp *urbp)
{
	...
	vusb_put_ring(vdev, shadow, false);
